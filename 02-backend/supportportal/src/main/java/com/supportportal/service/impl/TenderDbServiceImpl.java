...
public class TenderDbServiceImpl implements TenderDbService {
    ...
    @Override
    public List<TenderDb> removeTenderDbsByDeadlineBefore(LocalDate date) {
        return tenderDbRepository.removeTenderDbsByDeadlineBefore(date);
    }

    private List<TenderDb> applyShowCpvFilters(String username, List<TenderDb> tenderDbs) {
        List<UserCpvShowDb> userCpvShowDbs = userCpvShowRepository.findAllByUsernameOrderByCpvDbIdAsc(username);
    ...
    private List<TenderDb> applyNotShowCpvFilters(String username, List<TenderDb> tenderDbs) {
        List<UserCpvNotShowDb> userCpvNotShowDbs = userCpvNotShowRepository.findAllByUsernameOrderByCpvDbIdAsc(username);
